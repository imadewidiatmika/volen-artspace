                  ...
                  <tbody>
                    @forelse($schedules as $schedule)
                      <tr>
                        ...
                        <td><a href="#" class="text-primary" onclick="showParticipants({{ $schedule->id }})">{{ $schedule->registered_participants ?? 0 }}</a></td>
                        <td><a href="#" class="text-primary">{{ $schedule->attended_participants ?? 0 }}</a></td>
                        <td>
                          <span class="badge bg-{{ $schedule->status === 'Open Registration' ? 'green' : ($schedule->status === 'Completed' ? 'blue' : ($schedule->status === 'Closed Registration' ? 'yellow' : 'red')) }}-lt">
                            {{ $schedule->status }}
                          </span>
                        </td>
                        <td>
        ...
        <div class="modal-content">
          ...
          <form id="scheduleForm">
            <div class="modal-body">
              ...
              <div class="mb-3"><label class="form-label">Activity</label><input type="text" id="activity" name="activity" class="form-control" placeholder="e.g., Clay Pottery Painting Basic" required></div>
